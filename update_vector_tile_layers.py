...
2. Add a blank layer layer to the map that is in the web mercator
...
def add_vector_tile_layers(vector_layers):
    ...
    for k, v in vector_layers.items():
        # First add a dummy layer to the map so that it sets the spatial reference
        # of the map to Web Mercator. This is nessecary to publish to the AGOL/
        # Google maps tiling scheme. 
        LOGGER.info(f'creating spatial reference layer for {k} group')
        arcpy.MakeFeatureLayer_management(SPATIAL_REFERENCE_LAYER, 'fc_set_spatial_reference')
        m.addDataFromPath(SPATIAL_REFERENCE_LAYER)
        LOGGER.info(f'added spatial reference layer for {k}')
        for i in v:
            layer = arcpy.mp.LayerFile(os.path.join(QDL_PATH, k, i + ".lyr"))
        ...
        arcpy.CreateVectorTilePackage_management(m, 
        f'mobile_suma_gis/vector_tile_packages/{k}.vtpk',
        'ONLINE',
        )
        LOGGER.info(f'finished creating vtpk')
        # Delete the old package and tile layer. The name of the old layer and new
        # layer must match or they will break links from web maps. AGOL does not 
        # currently support overwriting tile packages, only feature services
        LOGGER.info('Checkinig if content already exists on AGOL and' 
        'deleting if it does.')
        
        agol_items = gis.content.search(
            f'owner:jhth490 AND title:{k} AND group:{GROUP_ID}'
        )
        if len(agol_items) > 0:
        ...
        vtpk = gis.content.add(
            {'description':f'VTPK for the {k} group layer this layer is updated weekly.'},
            f'mobile_suma_gis/vector_tile_packages/{k}.vtpk',
            folder='vector_tiles'
        )
        ...
        # Remove the layers. All active layers in the map will be included in the vtpk
...
if __name__ == '__main__':
    try:
        main()
        LOGGER.warning('upload_to_agol script ran successfully')
